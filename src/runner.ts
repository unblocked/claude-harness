...
- For a build, test suite or other command that takes more than a minute or so: start it backgrounded and do other useful work (read the next file, plan the next step) while it runs, rather than writing a wait-loop that blocks the turn on it (\`for i in ...; do ...; sleep N; done\`). A blocking wait-loop costs wall-clock time for nothing in return; backgrounding does not.
